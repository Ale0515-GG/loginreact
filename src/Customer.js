...
const Customer = () => {
    ...
    const GetUserAccess = () => {
        ...
        fetch("http://localhost:8000/roleaccess?role=" + userrole + "&menu=customer").then(res => {
            if (!res.ok) {
                navigate('/');
            toast.warning('No estás autorizado a acceder');
                return false;
            }
            return res.json();
        }).then(res => {
    ...
    const handleadd = () => {
        ...
        toast.success('added')
            ...
            toast.warning('No tienes acceso para agregar');
        }
    }
    const handleedit = () => {
        if(haveedit){
        toast.success('edited')
            ...
            toast.warning('No tienes acceso para Editar');
        }
    }
    ...
    const handleremove = () => {
        ...
        toast.success('removed')
            ...
            toast.warning('No tienes acceso para eliminar');
        }
    }
